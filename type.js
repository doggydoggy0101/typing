...
// enter to open language area
document.getElementById('show-lang').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    showLangCenter();
    inputField.focus();
  }
...
function showLangCenter() {
  document.getElementById('lang-center').classList.remove('hidden');
  document.getElementById('command-center').classList.add('hidden');
}

function hideLangCenter() {
  document.getElementById('lang-center').classList.add('hidden');
  document.getElementById('command-center').classList.remove('hidden');
}
      ...
      if (response.status === 200) {
        response
          ...
          .then(body => {
            ...
            let keys_ = Object.keys(langs);
            let i;
            for(i = 0;i < keys_.length; i ++){

              let lang = document.createElement('div');
              lang.setAttribute('class', 'lang-button');
              lang.setAttribute('onClick', `setLanguage('${keys_[i]}')`);
              lang.setAttribute('id', keys_[i]);

              // set tabindex to current theme index + 4 for the test page
              lang.setAttribute('tabindex', i + 4);
              lang.addEventListener('keydown', e => {
                if (e.key === 'Enter') {
                  setLanguage(lang.id);
                  lang.style.background = langs[keys_[i]]['color'];
                  lang.style.color = langs[keys_[i]]['background'];
                  inputField.focus();

                }
              })

              if(langs[keys_[i]]['customHTML'] != undefined){
                lang.style.background = langs[keys_[i]]['background'];
                lang.innerHTML = langs[keys_[i]]['customHTML']
              }else{
                lang.textContent = keys_[i];
                lang.style.background = langs[keys_[i]]['background'];
                lang.style.color = langs[keys_[i]]['color'];
              }
              document.getElementById('lang-area').appendChild(lang);
            }
          })
          .catch(err => console.error(err));
      } else {
        console.log(`Cant find text.json`);
      }
    })
    .catch(err => console.error(err));
}